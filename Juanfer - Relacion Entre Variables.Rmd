...
url_servicios <- "http://microdatos.dane.gov.co/index.php/catalog/678/download/13726"
...
ruta_destino_servicios <- "./Datos/Servicios.zip"
...
nombre_servicios <- "Servicios del hogar.csv"
...
descargar_bd(url_servicios, ruta_destino_servicios, nombre_servicios)
...
csv_servicios <- read.csv2(file = paste("./Datos/", nombre_servicios, sep = ""), sep = ";", dec = ",")

#csv_caracteristicas <- csv_caracteristicas %>% filter(ORDEN == 1 & SECUENCIA_P == 1)
#csv_trabajo <- csv_trabajo %>% filter(ORDEN == 1 & SECUENCIA_P == 1)
#csv_educacion <- csv_educacion %>% filter(ORDEN == 1 & SECUENCIA_P == 1)
#csv_vivienda <- csv_vivienda %>% filter(ORDEN == 1 & SECUENCIA_P == 1)

#csv_caracteristicas %>% filter(ORDEN == 1 & SECUENCIA_P == 1) -> csv_caracteristicas_aux
#csv_trabajo %>% filter(ORDEN == 1 & SECUENCIA_P == 1) -> csv_trabajo
#csv_educacion %>% filter(ORDEN == 1 & SECUENCIA_P == 1) -> csv_educacion
#csv_vivienda %>% filter(ORDEN == 1 & SECUENCIA_P == 1) -> csv_vivienda
```

Corrigiendo el nombre de la primera columna para todos los csv y observando el resultado
```{r}
#Se cambia el nombre de la primera columna
colnames(csv_caracteristicas)[1] <- "Directorio"
colnames(csv_caracteristicas_aux)[1] <- "Directorio"
...
### Características y Composición del Hogar
```{r}
colnames(csv_caracteristicas)
```

### Fuerza de Trabajo
```{r}
colnames(csv_trabajo)
```

### Educación
```{r}
colnames(csv_educacion)
```
...
jefe_hogar <- csv_caracteristicas_aux %>% filter(P6051 == 1 & SECUENCIA_P == 1)
...
ggplot(auxiliar_merge, aes(x = as.factor(P8634), y = Hijos)) +  geom_boxplot() + xlab("P8634")
```
...
```
```{r}
ggplot(hijos_jefe, aes(x = as.factor(P5502), y = Hijos)) + geom_boxplot() + xlab("P5502")
```
...
ggplot(auxiliar_merge, aes(x = as.factor(P6218), y = Hijos)) +  geom_boxplot() + xlab("P6218")
```
...
ggplot(auxiliar_merge, aes(x = as.factor(P8520S1), y = Hijos)) +  geom_boxplot() + xlab("P8520S1")
```
...
ggplot(auxiliar_merge, aes(x = as.factor(P1070), y = Hijos)) +  geom_boxplot() + xlab("P1070")
```