...
```{r}
BASE_FINAL %>% 
  select(all_of(variables_definitivas)) -> BASE_FILTRADA 
```
...
```{r}
nombres <- c("Hijos","nivel_edu","estu_actual","tipo_trabajo","num_cuartos","num_sanitarios",
             "estado_civil","comidas_dia","I_HOGAR","PERCAPITA","I_OU","CANT_PERSONAS_HOGAR",
             "razon_no_estudie","consumo_energia","tipo_vivienda","sexo","conyugue_hogar","edad_jefe",
             "estado_salud","regimen_salud","edad_conyugue")
length(nombres)
colnames(train) <- nombres
colnames(test) <- nombres
```
