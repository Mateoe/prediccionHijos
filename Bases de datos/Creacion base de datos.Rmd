...
```{r}
# Obteniendo la Edad del conyugue
compo_hogar %>% 
  filter(P6051==2) %>% 
...
BASE_FILTRADA <- Reduce(function(x, y) merge(x, y, all.x = TRUE), list(BASE_FILTRADA,
                                                                       ...
                                                                       ed_max,
                                                                       pareja))
# Los hogares que hayan quedado con NA's tendran cero hijos 
...
# NA's en regimen salud se les agrega a la categoría 9
BASE_FILTRADA$regimen_salud[is.na(BASE_FILTRADA$regimen_salud)] <- 9
# NA's en si el conyugue vive en el hogar se les agrega a la categoría 0
