...
hijos_hogar <- compo_hogar %>% select(clave, P6051) %>% filter(P6051 == 3) %>% count(clave)
...
```{r}
mean_P6080 <- hijos_jefe %>% 
  select(P6080,Hijos) %>% 
  filter(!is.na(P6080)) %>% 
  group_by(P6080) %>% 
  dplyr::summarize(Hijos = mean(Hijos,na.rm=TRUE))
...
ggplot(hijos_jefe, aes(x = as.factor(P6080), y = Hijos)) +
    ...
    geom_point(mean_P6080, mapping = aes(x = as.factor(P6080), y = Hijos),
               col = "Red")
```

```{r}
ggplot(subset(hijos_jefe, !is.null(P6080)), aes(x = as.factor(P6080))) +
    geom_bar()
```
...
ggplot(subset(hijos_jefe, !is.na(P6087)), aes(x = as.factor(P6087), y = Hijos)) +
    ...
    theme(plot.title = element_text(hjust = 0.5))+
    geom_point(mean_P6087, mapping = aes(x = as.factor(P6087), y = Hijos),
               col = "Red")
```

```{r}
ggplot(subset(hijos_jefe, !is.null(P6087)), aes(x = as.factor(P6087))) +
    geom_bar()
```
...
ggplot(subset(hijos_jefe, !is.na(P6020)), aes(x = as.factor(P6020), y = Hijos)) +
    ...
    theme(plot.title = element_text(hjust = 0.5))+
    geom_point(mean_P6020, mapping = aes(x = as.factor(P6020), y = Hijos),
               col = "Red")
```

```{r}
ggplot(subset(hijos_jefe, !is.null(P6020)), aes(x = as.factor(P6020))) +
    geom_bar()
```
...
```{r}
mean_P8520S3 <- hijos_jefe %>% 
  select(P8520S3,Hijos) %>% 
  filter(!is.na(P8520S3)) %>% 
  group_by(P8520S3) %>% 
  dplyr::summarize(Hijos = mean(Hijos,na.rm=TRUE))
...
ggplot(hijos_jefe, aes(x = as.factor(P8520S3), y = Hijos)) +
    ...
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_point(mean_P8520S3, mapping = aes(x = as.factor(P8520S3), y = Hijos),
               col = "Red")
  
```

```{r}
ggplot(subset(hijos_jefe, !is.null(P8520S3)), aes(x = as.factor(P8520S3))) +
    geom_bar()
```
...
Los ecuestados responden a la siguente pregunta: **En general, qué tan satisfecho(a) se siente ... con su trabajo/actividad actualmente?**
...
```{r}
#Resumenes numericos
resumen_numerico <- tapply(hijos_jefe$Hijos, hijos_jefe$P1899, summary)
...
mean_P1899 <- hijos_jefe %>% 
  select(P1899,Hijos) %>% 
  filter(!is.na(P1899)) %>% 
  group_by(P1899) %>% 
  dplyr::summarize(Hijos = mean(Hijos,na.rm=TRUE))

ggplot(subset(hijos_jefe, !is.na(P1899)), aes(x = as.factor(P1899), y = Hijos)) +
    geom_boxplot() +
    xlab("Satisfacción con la actividad/trabajo que realiza") + 
    ggtitle("Número de hijos según la satisfacción con \n la actividad/trabajo que realiza")+
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_point(mean_P1899, mapping = aes(x = as.factor(P1899), y = Hijos),
               col = "Red")
...
```{r}
ggplot(subset(hijos_jefe, !is.null(P1899)), aes(x = as.factor(P1899))) +
    geom_bar()
```
...
Creación del boxplot
```{r}
mean_P5095 <- hijos_jefe %>% 
  select(P5095,Hijos) %>% 
  filter(!is.na(P5095)) %>% group_by(P5095) %>% 
  dplyr::summarize(Hijos = mean(Hijos,na.rm=TRUE))
...
ggplot(subset(hijos_jefe, !is.null(P5095)), aes(x = as.factor(P5095), y = Hijos)) +
    ...
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_point(mean_P5095, mapping = aes(x = as.factor(P5095), y = Hijos),
               col = "Red")
```

```{r}
ggplot(subset(hijos_jefe, !is.null(P5095)), aes(x = as.factor(P5095))) +
    geom_bar()
```
```{r}
hijos_jefe[hijos_jefe$Directorio == 7121808,]
```
...
**Duda con el comportamiento de la media.**
...
```{r}
#Resumenes numericos
resumen_numerico <- tapply(hijos_jefe$Hijos, hijos_jefe$P6071, summary)
...
```{r}
mean_P6071 <- hijos_jefe %>% 
  select(P6071,Hijos) %>% 
  filter(!is.na(P6071)) %>% 
  group_by(P6071) %>% 
  dplyr::summarize(Hijos = mean(Hijos,na.rm=TRUE))

ggplot(subset(hijos_jefe, !is.na(P6071)), aes(x = as.factor(P6071), y = Hijos)) +
    geom_boxplot() +
    xlab("Conygue vive en el hogar") + 
    ggtitle("Número de hijos si el conyugue vive en el hogar")+
    theme(plot.title = element_text(hjust = 0.5))+
    geom_point(mean_P6071, mapping = aes(x = as.factor(P6071), y = Hijos),
               col = "Red")
```
